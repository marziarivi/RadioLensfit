...
#pragma omp parallel for
    ...
    set_posterior_values(numR, L_r, 0., par->rprior, par->ro, xmarvals, ymarvals, &numvals);
                       ...
                       double* uu_metres, double* vv_metres, complexd* visData, complexd* visM)
{
    ...
    cross_correlation(nchannels, wavenumbers, n_uv_coords, count, uu_metres, vv_metres, visData, visM, &ho, &det_sigma);
                       ...
                       complexd* visMod, double* ho, double* det_sigma)
{
    ...
    for (unsigned int ch=0; ch<nchannels; ch++)
    {
        ...
        for (i=0; i < n_uv_coords; i++)
        {
            ...
            d2h_dx2_xo += h_F[k].real * wavenumber*wavenumber * u*u;
            d2h_dy2_yo += h_F[k].real * wavenumber*wavenumber * v*v;
            d2h_dxdy_o += h_F[k].real * wavenumber*wavenumber * u*v;
            k++;
        }
    }
     ...
     */
    double xo=0., yo=0., incx=0., incy=0.;
...
int likelihood_sampling(int rank, double *mes_e1, double *mes_e2, double maxL, void *params, int np_max, double *var_e1, double *var_e2, double *cov_e)
{
    ...
    *cov_e = cov;
    ...
    if (var1 < 1e-10 || var2 <1e-10) return 1;
    else return 0;
